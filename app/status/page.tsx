...
const calculateShard = (guildId: string, totalShards: number): number => {
  const id = BigInt(guildId);
  const shardId = id >> BigInt(22);
  return Number(shardId % BigInt(totalShards));
};
...
const fetchStats = async () => {
  const [botRes, shardsRes, guildsRes] = await Promise.all([
    fetch(`${API}/bot`, {
      cache: "no-store",
    }),
    fetch(`${API}/shards`, {
  ...
  if (!botRes.ok || !shardsRes.ok || !guildsRes.ok) {
  ...
  for (const shard of shards) {
    ...
    shardStats[shard.id] = {
      ...
      users: shardGuilds.reduce(
        (acc: number, g: any) => acc + (g.members || 0),
        0
      ),
    };
  }
...
export default function StatusPage() {
  ...
  const [highlightedShard, setHighlightedShard] =
  ...
  const searchResult = useMemo(() => {
    if (!searchQuery.trim() || !stats?.shards) {
      return null;
    }

    const guildId = searchQuery.trim();

    if (!/^\d+$/.test(guildId)) {
      return null;
    }

    const targetShard = calculateShard(
      guildId,
      stats.shards
    );
  ...
  const scrollToShard = () => {
    if (searchResult) {
      const shardElement =
        shardRefs.current[searchResult.targetShard];
  ...
  useEffect(() => {
    if (searchResult) {
      setHighlightedShard(searchResult.targetShard);
    } else {
        ...
        {isLoading ? (
          ...
          <div className="space-y-6">
            ...
            <div className="grid gap-3 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
              {Object.entries(
                stats.shard_stats || {}
              ).map(([id, shard]: [string, any]) => {
                const isHighlighted =
                  highlightedShard === shard.id;

                return (
                  <div
                    key={id}
                    ref={(el) => {
                      shardRefs.current[shard.id] = el;
                    }}
